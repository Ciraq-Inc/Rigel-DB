            ...
            <table class="table table-striped mt-2">
              ...
              <tbody>
                <tr v-for="company in companyServer" :key="company.id">
                  ...
                  <td class="text-center">{{ company.localdbport }}</td>
                ...
                <div class="row justify-content-between text-left my-4">
                  <div class="form-group col-sm-6 flex-column d-flex">
                    ...
                    <input autofocus="autofocus" type="text" id="username" name="username" placeholder=""
                      v-model.trim="localCompany.localdbport" required>
                  </div>
                  <!-- R-HOST -->
                  <div class="form-group col-sm-6 flex-column d-flex">
    ...
    async fetchServers() {
      try {
        ...
        if (response.data.sucessful) {
          this.companyServer = response.data.data;
          console.log('Servers fetched successfully');
        } else {
          console.log('Could not fetch servers', response.data.message);
        }
      } catch (err) {
